"""
SAML Response Generator - Signs BOTH Response AND Assertion
Maximum compatibility - works with any Okta signature verification setting
"""
import base64
import uuid
...
def create_saml_response_simple(entity_id, acs_url, request_id, audience, user_email,
    ...
    """Create SAML Response - Signs BOTH Assertion AND Response for maximum compatibility"""
    ...
    # Sign BOTH Assertion AND Response for maximum compatibility
    if cert and key:
        ns = {'saml': 'urn:oasis:names:tc:SAML:2.0:assertion'}

        # STEP 1: Sign the Assertion first
        assertion_elem = response_elem.find('.//saml:Assertion', namespaces=ns)

        if assertion_elem is not None:
            print("[DEBUG] Step 1: Signing Assertion...")
            signer = XMLSigner(
            ...
            print("[DEBUG] ✓ Assertion signed")

        # STEP 2: Sign the Response (with the already-signed Assertion inside)
        print("[DEBUG] Step 2: Signing Response...")
        signer2 = XMLSigner(
            method=methods.enveloped,
            signature_algorithm='rsa-sha256',
            digest_algorithm='sha256'
        )
        response_elem = signer2.sign(response_elem, key=key, cert=cert)
        print("[DEBUG] ✓ Response signed")
        print("[DEBUG] ✅ BOTH elements signed - Maximum compatibility!")

        response_xml = etree.tostring(response_elem, pretty_print=True, xml_declaration=True, encoding='UTF-8')
    else:
        print("[DEBUG] ⚠️  Warning: No cert/key - response will be unsigned")
